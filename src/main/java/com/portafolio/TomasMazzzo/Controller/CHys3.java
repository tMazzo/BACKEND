...
public class CHys3 {
   ...
   @CrossOrigin(origins = "*")
    ...
    // CREAR EXPERIENCIA
    @PostMapping("/create")
    public ResponseEntity<?> create(@RequestBody dtoHys3 dtohys, HttpServletResponse response) {
    response.setHeader("Access-Control-Allow-Origin", "*");
        ...
        Hys3 hys = new Hys3(dtohys.getNombreS(), dtohys.getColorS(), dtohys.getPorcentajeS());
        sHys.save(hys);
    ...
    public ResponseEntity<?> update(@PathVariable("id") int id, @RequestBody dtoHys3 dtohys) {
